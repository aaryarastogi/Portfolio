    ...
    <motion.nav
     className={`flex h-auto justify-between fixed w-full top-0 md:flex md:items-center z-20 backdrop-blur-sm bg-transparent`}>
      ...
      <ul className={`md:flex hidden md:items-center bg-transparent backdrop-blur-sm text-black lg:mx-8 gap-6 md:z-auto md:static absolute w-full left-0 md:justify-end md:mr-10 md:py-0 py-4 md:pl-0 pl-6 md:opacity-100 ${open ?'opacity-100':'opacity-0'} transition-all ${open?'top-14' :'top-[-400px]' } ease-in duration-500 font-semibold`}>
        ...
        <li><Link to='/' className='md:text-xl font-semibold text-sm text-white hover:text-blue-600 backdrop-blur-sm  transition-all duration-100 ease-in-out' onClick={handleContact}>Contact Us</Link></li>
      </ul>
    </motion.nav>
  );
}
